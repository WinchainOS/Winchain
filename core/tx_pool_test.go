...
func setupWinchainTxPool() (*TxPool, *ecdsa.PrivateKey) {
	db := ethdb.NewMemDatabase()
	statedb, _ := state.New(common.Hash{}, state.NewDatabase(db))
	blockchain := &testBlockChain{statedb, uint64(1000000), new(event.Feed)}

	key, _ := crypto.GenerateKey()
	pool := NewTxPool(testTxPoolConfig, params.WinchainTestChainConfig, blockchain)

...
// Current   information for building the next block
type work struct {
...
type minter struct {
	...
	// 环形channel，长度为1， 防止重复提交挖矿申请
	shouldMine       *channels.RingChannel
	blockTime        time.Duration

	// 监听到新的chainHeadSub 事件时，没有上链时，区块暂存位置。
...
func (minter *minter) createWork() *work {
	...
	//parent, parent.GasLimit(), parent.GasLimit()
	header := &types.Header{
	...
	statedb, err := minter.chain.StateAt(parent.Root())
	if err != nil {
...
func (minter *minter) mintNewBlock() {
	...
	//if _, err := work.publicState.Commit(minter.chainDb, deleteEmptyObjects); err != nil {
	if _, err := work.publicState.Commit(deleteEmptyObjects); err != nil {
		panic(fmt.Sprint("error committing public state: ", err))
	}

	// TODO【Winchain】================the part had been changed==================
	//if _, privStateErr := work.privateState.Commit(minter.chainDb, deleteEmptyObjects); privStateErr != nil {
...
func (env *work) commitTransaction(tx *types.Transaction, bc *core.BlockChain, gp *core.GasPool) (*types.Receipt, *types.Receipt, error) {
	...
	//receipt, _, err := core.ApplyTransaction(env.config, bc, author, gp, env.publicState, env.privateState, env.header, tx, env.header.GasUsed, vmConf)

...
func openWinchainRaftDb(path string) (db *leveldb.DB, err error) {
...
func (pm *ProtocolManager) loadAppliedIndex() uint64 {
	dat, err := pm.winchainRaftDb.Get(appliedDbKey, nil)
...
func (pm *ProtocolManager) writeAppliedIndex(index uint64) {
	...
	pm.winchainRaftDb.Put(appliedDbKey, buf, noFsync)
}

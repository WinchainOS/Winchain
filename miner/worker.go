...
import (
	...
	"github.com/WinchainOS/Winchain/consensus/misc"
...
func newWorker(config *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, recommit time.Duration, gasFloor, gasCeil uint64) *worker {
	...
	//if _, ok := engine.(consensus.Istanbul); ok || !config.IsWinchain {
	if config.Raft == nil {
		// Not raft
...
func (w *worker) start() {
	...
	//TODO【xwc1125】================the part had been changed==================
...
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	for {
		...
		case <-timer.C:
			...
			if w.config.IsWinchain && w.isRunning() {
	...
	for {
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil  {
				...
				//else if w.config.Istanbul != nil && w.config.Istanbul.Period == 0 {
				//	w.commitNewWork(nil, false, time.Now().Unix())
				//}
			}
...
func (w *worker) commitTransactions(txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	for {
		...
		//TODO【Winchain】================the part had been changed==================
		// 判断交易是否是发布合约，并且是管理员发布的==》针对wpoa，需要过滤创世块
		if wpoa, ok := w.engine.(consensus.Wpoa); ok {
			if err := wpoa.ValidateTransaction(w.chain, w.chain.CurrentBlock(), tx); err != nil {
				log.Error("Err manager", "from", from, "error", err)
				txs.Pop()
				continue
			}
		}
